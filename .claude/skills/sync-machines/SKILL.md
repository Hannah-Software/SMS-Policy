...
expected_servers = {
    ...
    "gmail": {
        "type": "stdio", "command": "npx",
        "args": ["-y", "@gongrzhe/server-gmail-autoauth-mcp"], "env": {}
    },
    "postgres": {
        "type": "stdio", "command": "npx",
...
**MCP servers managed:**
- Linear (`@mseep/linear-mcp`) — key from Doppler `claude-code/prd`
- Gmail (`@gongrzhe/server-gmail-autoauth-mcp`) — OAuth via `~/.gmail-mcp/`
- Postgres — connection string from Doppler `claude-code/prd`
...
Key Doppler projects that need setup: `claude-code`, `opskpi`, `aipex-growth-agent`, `personal-agent`.

After login, verify critical secrets are accessible:
```bash
doppler secrets get LINEAR_API_KEY --project claude-code --config prd --plain 2>&1 | head -c 20
doppler secrets get ASANA_PAT --project claude-code --config prd --plain 2>&1 | head -c 20
```
...
for server in github figma taskmaster sqlite playwright sequential-thinking memory puppeteer linear filesystem gmail postgres; do
  python3 -c "
  ...
  cat > "$SETTINGS_FILE" << 'JSON'
{
  ...
  "permissions": {
    "allow": [
      ...
      "mcp__github__*", "mcp__linear__*", "mcp__memory__*",
